...
function buildPersonalitySection(dealer, channel, customerName) {
...
function buildJsonInstructionBlock() {
...
function formatHistory(messages, maxItems = 8) {
...
export async function bdcCustomerDiscovery(state) {
...
`; // Start simple; can be enriched when state includes goals
    ...
    const completion = await model.invoke([
        ...
        { _getType: () => "human", get type() { return "human"; }, content: userBlock },
        ...state.messages,
    ]);
    ...
    return { messages: [{ _getType: () => "ai", get type() { return "ai"; }, content: text }] };
}
export default bdcCustomerDiscovery;
//# sourceMappingURL=bdcCustomerDiscovery.js.map